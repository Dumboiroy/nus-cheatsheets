...
\begin{multicols*}{4}
  ...
  \end{itemize}
  \begin{tabular}{l}
    \includegraphics[width=0.7\linewidth]{stack-memory}
  \end{tabular}
  \begin{minipage}{0.15\columnwidth}
    Note: stack grows upwards in this case
  \end{minipage}
  ...
  \begin{itemize}
    ...
    \item \textbf{Utilization}: all parts of CPU should be utilized
  \end{itemize}
  ...
  Criteria:
  \begin{itemize}
    \item \textbf{Turnaround time}: Total time taken = finTime - arrTime
    \item \textbf{Waiting time}: Ready but cannot get CPU = TurnaroundT - Ideal - arr
    \item \textbf{Response time}: First CPU Time - Arrival Time
    \item \textbf{Throughput}: Rate of task completion = nTasksFinished/time
    \item \textbf{CPU Utilisation}: \% of time when CPU is working on a task
  \end{itemize}
  \subsubsection{First-Come First-Served (FCFS)}
  \begin{itemize}
    \item Batch processing, non-preemptive
    ...
    \item Guaranteed to have \textbf{no starvation}: no of tasks in front of task X in FIFO is always decreasing -> task X will get its chance eventually.
  ...
  \begin{itemize}
    \item Batch processing, non-preemptive
    \item Select the task with the smallest total CPU time, thus guaranteeing \textbf{smallest average waiting time}.
    \item Shortcomings: Need to know total CPU time for a task in advance (have to guess if not available), \textbf{starvation is possible} (biased towards short jobs, long jobs may never get a chance)
  ...
  \begin{itemize}
    \item Batch processing, preemptive
  ...
  \begin{itemize}
    \item General idea: Process $p_1$ creates a shared memory region (master process) $M$, process $p_2$ attaches $m$ to its own memory space. $p_1$ and $p_2$ can now commmunicate suing memory region $M$
    ...
    \item In SysV: (1) create shared memory region $M$ \texttt{shm\_get()}, (2) Attach $M$ to process memory space \texttt{shmat()}, (3) Read/Write $M$, (4) Detach $M$ from memory space after use\texttt{shmdt()}, (5) Destroy $M$ \texttt{shmctl()} (any 1 process with permission (usually master), can only destroy if $M$ is not attached)
    \item In POSIX: create shm\_open() -> size setup ftruncate() -> map/attach mmap() -> read/write -> unmap/detach munmap() -> close() -> unlink shm\_unlink()
  \end{itemize}
  ...
  \begin{itemize}
    \item There is a Race Condition when:
    \item 2/more processes execute concurrently in interleaving fashion AND share a modifiable resource resulting in non-deterministic execution.
  ...
  \begin{itemize}
    ...
    \item To stop code before critical section: while (TestAndSet(Lock) == 1);
  \end{itemize}
...
flag[1] = false;
  ...
  \begin{itemize}
    ...
    \item Binary semaphore, $S = 0$ or $1$ known as mutex (mutual exclusion)\\
  ...
  \subsubsection{Producer Consumer, Blocking Version}
  % \begin{tabular}{l}
    \includegraphics[width=0.9\linewidth]{busy-waiting}
  % \end{tabular}
  \begin{minipage}{\linewidth}
    Initial Values: count = in = out = 0; mutex = S(1), notFull = S(K), notEmpty = S(0)
  \end{minipage}
  \subsubsection{Tanenbaum Solution}
    \includegraphics[width=0.5\linewidth]{tanenbaum-1}
    \includegraphics[width=0.5\linewidth]{tanenbaum-2}
    \includegraphics[width=0.5\linewidth]{tanenbaum-3}
    \includegraphics[width=0.5\linewidth]{tanenbaum-4}
  \begin{tabular}{l}
