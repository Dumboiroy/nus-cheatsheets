...
\end{minted}
  ...
  \subsubsection{Table: (Segment\#, Page Limit, Page Table Base)}
  \includegraphics[width=0.5\linewidth]{s}
  \includegraphics[width=0.5\linewidth]{swp}
  ...
  \section{Additional Notes}
    ...
    \subsection {Memory Calculations}
      \begin{itemize}
        \item \textbf{Physical Address} = [frame number] [offset] | \textbf{Virtual Address} = [page number] [offset]
        \item \textbf{Physical -> Virtual OR vice versa}:
        \item Offset = $\text{addr} \mod 2^{\text{n-offset-bits}}$
        \item $\frac{\text{p\_or\_vAddr } - \text{ Offset}}{2^{\text{n\_offset\_bits}}}$ = page OR frame number 
        \item \textbf{Page/Frame Number + offset -> P or V address}:
        \item $\text{address} = \text{page or frame number} * 2^{\text{offset bits}} + \text{Offset}$
      \end{itemize}
